...
class Model(object):
   ...
   #         logging.info("Reading model parameters from %s", ckpt.model_checkpoint_path)
    ...
    def test(self, data_path):
        ...
        #print('test s_gen ', s_gen, data_path, self.buckets)		
        ...
        for batch in s_gen.gen(1):
            ...
            print('test output  ground ',  output, ground)
...
def revert_lex(lex):
        ...
        for i, l in enumerate(labels):
            ...
            while ('1' in s) or('2' in s) or ('3' in s):				
                ...
                n=i+k
                s=str(n)											
                #print('convert_lex  l i n k: ', l, i, n, k)
            l_id.append(n)	
        ...
        for c in lex:
            ...
            if int(c) >3  :	
                    n=n+c				
                    #print('revert n=n+c  ', n)					
                    #n=i+3
                    #print('revert c ord(c) l i n : ', c, ord(c), l, i, n)
            else : 
                c_idx = int(n)	
                #for i, labels in enumerate(label_list):
                for i, label in enumerate(label_list):					
                    #print('c_idx, i, label[0], label ', c_idx, i, label[0], label)					
                    if (c_idx) == label[0] :
                    #if (c_idx-4)  == i :
                            l_new+= label[1]				
                            #l_new.append(label[0])
        #print ('revert() n  l_new label[0], label,' , n, l_new, label[0], label)
